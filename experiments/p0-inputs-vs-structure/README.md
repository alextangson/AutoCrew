...
- 18 条证据需求补回 16 条（2 条如实报空，写手被告知「不要编」）；镜头标注 0 处。
- 数字核验是**字符串级**（`5` 能命中 `15`）：6 篇 31 个数字串都能在材料里找到，只证明没有凭空出现的数字串，不证明同主张同单位——生产版按 P1 spec §4.4 用归一元组重做。补证配额配置字段名写错（codex 核出），实际跑的是默认 4 搜 6 读，16/18 是默认配额下的结果。
