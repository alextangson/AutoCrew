...
| `autocrew_cover_review` | Generate, review, revise, and approve content-driven cover images using the configured image provider and personal cover style. Actions: `create_candidates`, `get`, `revise`, `approve`, `platform_ratios`, `generate_ratios` (legacy alias) |
...
├── cover-style.json        # Personal cover identity, typography, texture and layer rules
├── covers/templates/       # Ordered identity/editorial reference photos
