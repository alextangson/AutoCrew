...
import { parseImageChain } from "./settings.js";
import { resolveImageChain, providerLabel } from "../modules/publish/wechat-mp.js";

describe("parseImageChain", () => {
  it("解析一条完整的链并保序——顺序就是降级顺序", () => {
    const out = parseImageChain(JSON.stringify([
...
describe("resolveImageChain", () => {
  ...
  it("没配就是空链,不降级——与从前行为一致", () => {
    expect(resolveImageChain(undefined)).toEqual([]);
    expect(resolveImageChain([])).toEqual([]);
  });
});
...
describe("codex 通道（本地 Codex CLI，走 ChatGPT 订阅）", () => {
  it("codex 条目不需要 baseUrl/apiKey——它借的是已登录的订阅", () => {
    const out = parseImageChain(JSON.stringify([{ kind: "codex" }]));
    expect("value" in out && out.value).toEqual([{ kind: "codex" }]);
  });

  it("codex 可以排在中转前面——顺序就是优先级", () => {
    const out = parseImageChain(JSON.stringify([
      { kind: "codex" },
      { name: "xiaojiu", baseUrl: "https://api.xiaojiu.one/v1", apiKey: "sk-a" },
    ]));
    expect("value" in out && out.value.map((f) => f.kind)).toEqual(["codex", "relay"]);
  });

  it("非法 kind 被拒绝", () => {
    const out = parseImageChain(JSON.stringify([{ kind: "gemini" }]));
    expect("error" in out && out.error).toContain("kind");
  });

  it("resolveImageChain 给 codex 条目默认命名为 codex", () => {
    expect(resolveImageChain([{ kind: "codex" }])).toEqual([{ name: "codex", kind: "codex" }]);
  });
});
