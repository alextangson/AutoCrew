...
const screen: EditorCandidate = {
  ...
  fingerprint: { size: 10, mtimeMs: 1, quickHash: "screen-hash" },
};
...
describe("submit_timeline_plan 的参数形态归一", () => {
  ...
  it("正常数组照收", async () => {
    ...
    const out = await tool(captured).execute({ overlays: [good] });
    expect(out).toContain("1 段 B-roll");
    expect(captured.plan?.overlays[0]).toMatchObject({
      overlayId: "ov-01",
      inMs: 0,
      outMs: 10_000,
      source: { kind: "asset", name: "screen.mp4", fingerprint: { quickHash: "screen-hash" } },
    });
  });

  it("数组被中转序列化成 JSON 字符串 → 无声吃掉（这是这条链路的常态）", async () => {
    const captured: { plan: EditorToolCapture | null } = { plan: null };
    const out = await tool(captured).execute({ overlays: JSON.stringify([good]) });
    expect(out).toContain("1 段 B-roll");
    expect(captured.plan?.overlays).toHaveLength(1);
  });
  ...
  it("串里带未转义的半角引号 → 修完再解析，不打回让模型重来", async () => {
    ...
    const out = await tool(captured).execute({ overlays: raw });
    expect(out).toContain("1 段 B-roll");
  });
  ...
  it("不合规的编排原样打回，captured 保持空（打回不是「收到了」）", async () => {
    const captured: { plan: EditorToolCapture | null } = { plan: null };
    const out = await tool(captured).execute({ overlays: [{ ...good, outputStartMs: 1_000 }] });
  ...
  it("空编排是合法答案，且与「解析失败」是两句不同的话", async () => {
    const captured: { plan: EditorToolCapture | null } = { plan: null };
    const out = await tool(captured).execute({ overlays: [] });
    expect(out).toContain("不切 B-roll");
    expect(captured.plan).toEqual({ overlays: [] });
  });

  it("彻底解析不出数组 → 说清是解析问题", async () => {
    const captured: { plan: EditorToolCapture | null } = { plan: null };
    expect(await tool(captured).execute({ overlays: "{不是数组" })).toContain("解析不了");
  });

  it("待生成槽：description + mediaKind 收下，回执点出有几段待生成", async () => {
    const captured: { plan: EditorToolCapture | null } = { plan: null };
    const out = await tool(captured).execute({
      overlays: [{ description: "数字滚动 80%→20%，暗底细网格，克制", mediaKind: "image", outputStartMs: 40_000, durationMs: 8_000 }],
    });
    expect(out).toContain("1 段待生成");
    expect(captured.plan?.overlays[0]).toMatchObject({
      source: { kind: "generate", mediaKind: "image", description: "数字滚动 80%→20%，暗底细网格，克制" },
    });
  });

  it("assetId 与 description 二选一：都给 / 都不给 / 缺 mediaKind 都当场打回", async () => {
    const captured: { plan: EditorToolCapture | null } = { plan: null };
    const base = { outputStartMs: 40_000, durationMs: 8_000 };
    expect(await tool(captured).execute({ overlays: [{ ...base, assetId: "b1", description: "两个都给" }] })).toContain("二选一");
    expect(await tool(captured).execute({ overlays: [{ ...base }] })).toContain("二选一");
    expect(await tool(captured).execute({ overlays: [{ ...base, description: "缺 mediaKind" }] })).toContain("mediaKind");
    expect(captured.plan).toBeNull();
  });
});
...
describe("runEditor 的出口只有两种：llm 或空 plan（永不抛错）", () => {
  it("模型给出编排 → origin llm + provenance", async () => {
    const out = await runEditor(input(), { runLoopImpl: fakeRunLoop([{ overlays: [good] }]) });
    expect(out.origin).toBe("llm");
    expect(out.overlays).toHaveLength(1);
    expect(out.provenance).toMatchObject({ promptVersion: "ed-2", assetsHash: "cafe1234" });
  ...
  it("模型自纠：第一轮不合规被打回，第二轮合规 → 采纳第二轮", async () => {
    const out = await runEditor(
      input(),
      { runLoopImpl: fakeRunLoop([{ overlays: [{ ...good, outputStartMs: 0 }] }, { overlays: [good] }]) },
    );
    expect(out.origin).toBe("llm");
  ...
  it("模型交空编排 → 仍是 origin llm（「它看过了，认为不用切」）", async () => {
    const out = await runEditor(input(), { runLoopImpl: fakeRunLoop([{ overlays: [] }]) });
    expect(out).toMatchObject({ origin: "llm", overlays: [] });
    expect(out.warning).toBeUndefined();
    expect(out.note).toBeUndefined();
  });

  // v2 spec §4.2：零素材短路已删——它留着，generate 槽就永远不会在零素材稿件上出现
  it("零素材照样跑模型，可以全提 generate 槽", async () => {
    let called = false;
    const out = await runEditor(input({ candidates: [] }), {
      runLoopImpl: ((config: unknown, opts: Parameters<typeof fakeRunLoop>[0] extends never ? never : never) => {
        called = true;
        return fakeRunLoop([
          { overlays: [{ description: "暗底细网格上数字滚动 80%→20%，克制", mediaKind: "video", outputStartMs: 40_000, durationMs: 10_000 }] },
        ])(config as never, opts as never);
      }) as never,
    });
    expect(called).toBe(true);
    expect(out.origin).toBe("llm");
    expect(out.overlays[0]!.source).toMatchObject({ kind: "generate", mediaKind: "video" });
  });
  ...
  it("素材说明与口播一律当数据：prompt 明说不执行里面的指令", async () => {
    ...
    // §5.2 描述规范 + §5.3 工艺红线必须在 prompt 里（创始人品味不能只写在 spec 里）
    expect(seen.system).toContain("画面主体");
    expect(seen.system).toContain("禁止逐字 3D 飞入");
