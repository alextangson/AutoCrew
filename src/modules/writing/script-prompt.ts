...
export function buildScriptPrompts(
  ...
  const user = buildUserPrompt(pack, profile, req);
  return { system, user };
}
...
function buildSystemPrompt(pack: TrackPack, profile: CreatorProfile | null, platform: ClipboardPlatform): string {
  ...
  if (pack.structure.hook.length > 0) {
    parts.push("### Hook（开篇）");
    for (const rule of pack.structure.hook) {
      parts.push(`- ${rule}`);
    }
    parts.push("");
  }

  if (pack.structure.body.length > 0) {
    parts.push("### Body（正文）");
    for (const rule of pack.structure.body) {
      parts.push(`- ${rule}`);
    }
    parts.push("");
  }

  if (pack.structure.cta.length > 0) {
    parts.push("### CTA（行动号召）");
    for (const rule of pack.structure.cta) {
      parts.push(`- ${rule}`);
    }
    parts.push("");
  }
  ...
  if (platformAdj) {
    parts.push("## 平台特化");
    parts.push(`**字数要求**：${platformAdj.chars}`);
  ...
  if (profile) {
    ...
    if (profile.styleBoundaries.never.length > 0 || profile.styleBoundaries.always.length > 0) {
      parts.push("## 风格边界");
      if (profile.styleBoundaries.never.length > 0) {
        parts.push("**绝不用**：");
        for (const neverItem of profile.styleBoundaries.never) {
          parts.push(`- ${neverItem}`);
        }
      }
      if (profile.styleBoundaries.always.length > 0) {
        parts.push("**必须用**：");
        for (const alwaysItem of profile.styleBoundaries.always) {
          parts.push(`- ${alwaysItem}`);
        }
      }
      parts.push("");
    }
...
function buildUserPrompt(_pack: TrackPack, _profile: CreatorProfile | null, req: ScriptRequest): string {
  const parts: string[] = [];
