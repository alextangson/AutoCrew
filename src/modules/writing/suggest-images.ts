...
export async function suggestImagePositions(
  ...
  const systemPrompt = [
    ...
    "数量由文章决定，不是固定配额：每张图必须有明确职责——这段的机制/流程/对比/数据/关键场景，配图后读者理解得更快才插；找不到这样的段落就少插，一处都不值得就一处不插(照样调 submit_body 提交原文)。密度参考：约每 800-1000 字至多 1 处，全文最多 5 处；短文(1200 字内)至多 1-2 处。宁缺毋滥，不为凑数在纯过渡段插氛围图。",
    "通用规则：放在关键转折、数据展示、机制讲解或场景描写之后；不用写配色和画风(系统会统一处理)，你只负责画面内容和(解释图的)标签文字；保留原文一字不改，只插入标记，不删不改正文；已有的 [IMAGE:] 标记原样保留、计入总数。",
  ...
  if (added < 0) throw new Error("模型删掉了已有的插图标记，改动已丢弃，请重试");
