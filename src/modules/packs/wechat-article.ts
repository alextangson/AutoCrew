 ...
 * （P0 附录 0-2/0-3 处置清单）。Quality Gate 阈值 = 原 5 项硬门禁中可判定的
...
export const WECHAT_ARTICLE_PACK: TrackPack = {
  ...
  version: 1,
  writerRole:
    "你是一名公众号深度图文写手，擅长把一个选题写成 5000-6000 字、像朋友聊天一样好读、但论证扎实的长文。",
  reward: {
    default: {
      primary: "views",
  ...
  structure: {
    ...
    body: [
      "5000-6000 字深度长文：不是短文扩写，是完整论证",
      "至少 3 个具体案例（公司名/人名 + 事件），至少 5 处数据引用（数字、百分比、时间点、金额）",
      "口语化，像朋友聊天；短句为主；每 300 字左右一个呼吸点（换段）",
      "包含至少一个方法论框架或决策模型；补充反面观点和风险分析",
      "正文不用 markdown 标题/列表语法（公众号不支持），可用加粗与分隔线",
      "在数据对比、流程说明、概念示意、案例说明处插入 [IMAGE: 具体画面prompt] 配图标记，全文至少 4 个、分布在不同段落",
    ],
    cta: [
  ...
  platformAdjustments: {
    wechat_mp: {
      chars: "5000-6000",
      style: "深度图文；口语化短句；[IMAGE:] 标记保留原样，发布链负责生成配图",
    },
  },
  qualityGate: {
    minChars: 5000,
