...
def get_access_token():
    ...
    # AutoCrew 经环境变量注入真实凭证（设置→发布）；config.json 里的占位值退居兜底。
    app_id = os.environ.get("WECHAT_APP_ID") or wechat.get("app_id")
    app_secret = os.environ.get("WECHAT_APP_SECRET") or wechat.get("app_secret")

    if not app_id or not app_secret:
        print("错误: 未配置公众号 app_id / app_secret（AutoCrew 设置→发布，或 config.json）")
        sys.exit(1)
