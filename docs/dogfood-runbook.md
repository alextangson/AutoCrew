...
| 现象 | 原因 | 修复 |
|---|---|---|
| 反复弹「找不到用于储存 Electron Key 的钥匙串」| 用 `HOME=/tmp/...` 隔离环境启动的测试实例——macOS 按 `$HOME/Library/Keychains` 找钥匙串，HOME 一改就找不到 | 弹窗点**取消**（别点「还原为默认」）；测试/CI 启动一律加 `--use-mock-keychain` 旗标（CDP 冒烟脚本已强制） |
...
> **S2.8 已升级为任务工作台形态**：见第十二章。左侧边栏 + 中央 hero/会话 + 按需右侧滑出抽屉，原「右工作区」各面板改由抽屉承载；本节描述的 Sprint 1 能力不变。

- **日常路径**：直接对话——「帮我写一条关于 X 的抖音口播」→ 稿件卡 →「排版发布文案」→ 复制 → 平台粘贴发布 → 卡上点「我已发布，确认」。
- **风格纠偏**：对话里直接说（「太 AI 味」「口语一点」）→ 自动记为永久规则；侧边栏「数字员工」→「编剧」抽屉可编辑/停用每条风格规则。
- **首跑**：自动进 Day-1 onboarding（赛道 → CSV 导入 → 风格校准 → playbook 报告），每步可跳过，聊天输入框全程可用。
- **引擎配置**：侧边栏底部「设置」→ 开发者区（折叠）→ API key / Base URL / 模型档位，落盘 `~/.autocrew/engine.json`（自动 chmod 0600；终端用户版本此区隐藏——§9 薄云中转上线后退役）。原 env / engine.json 手工方式仍兼容；key 在 UI 中永远只显示掩码。
- **通道清单**：41 条 IPC（见 `src/desktop/channels.ts`；S3.0 新增 `today:summary` 聚合通道）；`dialog:pick_file` 仅主进程可用；`chat:turn` 的工具 args 经 sanitize 剥 `_` 前缀键（防模型注入 `_dataDir`）。
...
## 十二、任务工作台（S2.8，2026-06-11）

S2.8 把 IA 重构为 Accio Work 式三件套：**左侧边栏 + 中央 hero/会话 + 按需右侧滑出抽屉**，并落地对话持久化（会话=任务，重启可回放）。crew-bar 退役，功能并入侧边栏「数字员工」区。

### 新建任务

在中央 hero 输入框输入第一条消息（如「帮我写一条关于 AI 时代普通人最该练的技能的抖音口播」）→ 发送后自动建任务，首轮成功后会话落盘。后续对话均追加到同一任务，无需任何手动操作。

- 输入框上方的**上下文 chip**（赛道 / 知识库状态）可点击打开对应抽屉——赛道来自 onboarding 档案，知识库状态显示已索引文件数。
- busy 期间（模型跑回合中），新建任务/切换会话/删除均被挡住，会有 toast 提示；等回合完成再操作。

### 任务历史

左侧边栏「任务历史」区列出所有历史会话，按最近更新时间排序。

- **点击任意条目**：中央区切换为该任务的历史消息，含完整卡片（稿件卡/归因卡等）原样回放。
- **hover 条目** → 出现 ✕ 删除按钮 → 点击弹确认对话框 → 确认后删除，对应 `~/.autocrew/conversations/<id>/` 目录及其内容一并删除。删除不可恢复。

### 数字员工入口

左侧边栏「数字员工」区列出四个角色：**侦察员**（选题雷达源 + 定位）、**编剧**（风格规则）、**审核员**（合规说明卡）、**分析师**（回流报告/指标偏好）。

- 点击任意角色 → 对应面板抽屉从右侧滑出。
- 「即将上线」区（定时任务/插件/消息渠道/团队）当前显示为禁用占位，不可点击。

### 抽屉操作

右侧抽屉（稿件工作台、回流报告、风格、设置、侦察员等）有三种关闭方式：

1. 按 **Esc** 键
2. 点击抽屉外的**遮罩区域**
3. 点击抽屉内的 **✕** 按钮

同时只能开一个抽屉；打开新抽屉会关闭当前已开的。
...
## 十三、素材库（S2.9，2026-06-11）

**核心语义：库是索引，项目是快照。**

- 入库 = 引用原始文件路径，**不复制文件**；库只保存元数据（路径、名称、标签、缩略图等），原文件留在用户自己的硬盘位置。
- 从素材库挂接到稿件 = **复制进项目目录**（快照），稿件内副本与库内原文件相互独立——删稿件副本不影响库，库内删记录不影响已挂接稿件。
- **删除素材只删记录，永不删原文件。**
- 原文件被移走或删除后，列表条目灰显「文件已移动或删除」，并提供「重新定位」按钮重新绑定路径——不会静默失效。

### 存储位置

```
~/.autocrew/library/
  folders.json          # 文件夹列表（扁平结构，含 id/name/createdAt）
  assets/
    <asset-id>.json     # 每条素材的元数据（id/path/name/type/tags/addedAt/thumbnailPath 等）
```

写入为原子操作（temp-rename），单条损坏跳过、不影响其他条目。

### 导入素材

1. 对话中说「往素材库加几个视频」或点 UI 入口 → 弹出多选文件对话框（经 `dialog:pick_media` IPC，路径走主进程媒体白名单收口）。
2. 选定文件后自动入库——记录原始路径，不做复制；重复路径幂等跳过。
3. 图片类型自动生成缩略图（CSP `img-src file:` 已实测可渲染）。

### 文件夹与标签

- **文件夹**：扁平结构，无嵌套。创建/删除文件夹不影响素材记录本身（素材的 folderId 字段更新/清空）。
- **标签**：自由文本多标签，逗号分隔，每条素材可带多个标签。

### 搜索与筛选

- **搜索**：名称 + 标签双轨匹配，实时过滤，不区分大小写。
- **类型筛选**：视频 / 图片 / 音频三类独立勾选，可组合。
- 两者可同时使用。
...
1. 打开素材库选择面板，搜索/筛选后点击目标素材。
2. 系统将原文件**复制进项目目录**（快照），挂接记录写入稿件元数据。
3. 文件 > 500 MB 时弹确认提示（「文件较大（X MB），确认复制？」），用户确认后继续。
4. 稿件内副本后续可独立移除，不影响库内原文件记录。

### 编剧侧工具：search_assets

编剧 agent 新增只读工具 `search_assets`，支持按名称/标签/类型检索素材库，返回素材卡（含路径、名称、标签、类型、缩略图 URL）。

```
# 示例：让编剧找所有带「产品」标签的视频
（对话）帮我找素材库里标签含"产品"的视频
```

编剧工具只读——不能通过对话修改或删除库内记录。

### 安全说明

- 媒体路径白名单在主进程层收口（`library:add` 和 `library:update`/relocate 两个写入口均经校验）。
- asset id 格式经正则校验，防路径穿越。
- 文件名在写入 JSON 前经防穿越处理。
- 记录守卫要求 `id / path / name / addedAt / tags` 字段齐全，缺字段记录跳过加载。

---

## 十四、创作者指挥台 / 今日工作台（S3.0，2026-06-14）

S3.0 把已有后端（雷达/流水线/飞轮/会话）暴露到创作者母语主屏——不再以对话为一级入口，而是以**创作者视角的内容工作流**重组 IA。

### 三模式导航

应用现在有三个主屏模式，通过左栏导航切换：

| 模式 | 触发 | 用途 |
|---|---|---|
| **今日** | 点左栏「今日」/ 启动默认页 | 首屏工作台（见下方四块） |
| **会话** | 点左栏任务条目 / 命令栏发消息 | chat-stream + 卡片，复用 S2.8 会话引擎 |
| **目的页** | 点左栏五入口之一（选题/内容/数据/素材）| 该领域的独立视图（面板从抽屉提升为中央） |

左栏五个入口：今日 / 选题 / 内容 / 数据 / 素材。＋新任务常驻顶部，设置在底部。

> **抽屉退役**：S2.8 的五面板抽屉（侦察员/稿件工作台/回流报告/风格/设置）已升级为目的页，不再从右侧滑出。旧侧边栏的「即将上线」「数字员工」区已不再占侧边栏位置。

### 今日首屏四块

#### 1. 数字员工存在感条
横幅展示当前活跃的数字员工角色（侦察员/编剧/审核员/分析师）。点击某个角色头像 → 直接进入该角色的目的页，无需通过侧边栏跳转。

#### 2. 选题雷达
展示热榜候选选题卡（来自 `getCachedTopicCandidates`，首屏只读缓存，**不联网**，缓存由 S1 选题雷达定时填充）。点「就这个写」→ 跳入会话模式，以该选题为第一条消息预填。

#### 3. 进行中
当前流水线状态一览：
- 统计各状态下的稿件计数（草稿/待审/已发布等，legacy 状态已做规范化）
- 超出正常时长的稿件触发「卡顿提醒」，提示关注

#### 4. 数据回流
上一条已发布稿件的核心数据快照：完播率 vs. 个人基线对比，一眼判断表现。

### 最近任务（Cursor 模型）

任务历史采 **Cursor 模型**：最近 5 条会话浮在今日首屏；全部会话可通过左栏任务列表访问，不常驻侧边栏固定区域。「删了一个任务，侧边栏不会空出奇怪的占位」——历史任务数量不影响主屏密度。

### 命令栏

今日首屏底部常驻命令栏（输入框）。在命令栏发消息 = 自动建新会话并进入会话模式，与 S2.8 的 hero 输入框语义一致，但现在是全局常驻（无论在哪个目的页均可快速发消息）。

### today:summary 聚合通道

后端新增 `today:summary` IPC（IPC 总数从 40 扩至 **41**），一次调用返回首屏全部数据：
- 当前各状态流水线计数
- 选题候选列表（缓存）
- 最近任务概要
- 上条已发布稿数据回流

首屏数据不分多次 IPC 请求——单次聚合降低渲染抖动。

### 设计文档

见 `docs/superpowers/specs/2026-06-14-sprint3.0-creator-command-center-design.md`（设计规格）和 `docs/superpowers/plans/2026-06-14-sprint3.0-creator-command-center.md`（实施计划）。
...
1. Report 页逐平台点「立即抓取」，看状态徽标与入库行数；
2. `ok + 行数>0` → 对照后台真实作品数，抽 2 条核对播放/点赞数值与**完播率量纲**（校准清单第 1 条，当前归一规则可能把 0.8% 放大成 80%）；
3. `ok + 0 行` 但后台明明有作品 → 大概率是「合法但为空」型静默错误（如视频号 `userpageType` 取值不对），按端点文档校准清单排查；
4. `schema_changed` → 接口漂移，零写入是设计行为；抓包比对 fixture 字段后更新守卫。
