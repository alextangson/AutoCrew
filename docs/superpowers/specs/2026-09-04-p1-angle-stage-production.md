...
## 12. P1b 落地记录（2026-09-05）

commits `745a9a8`（own-material / 账本 / 补证 / 数字门 / 格式门）、`1e267e2`（接进写稿主链：输入预算装配、补证、账本先落盘、find_evidence、`needs_evidence`、归因）、`4fa0b6f`（写手/修订 token 预算固定 120k——抖音无 gate 时原本落回 runLoop 默认 2 万，v3 角度块 + 12k 研究槽一轮吃光，写手连 submit 都没调到）。4152 测试绿。

**端到端验收**（隔离目录，DeepSeek V4 Pro）：「DeepSeek Harness」→ angles job 320s，4 张卡 3 张锚在创作者口播转写（P1a 时零锚点）→ 选涨粉画像亲历复盘卡 → 补证 3 条需求 found(4)/failed/empty → 写手用 `find_evidence` 1 次 → 数字门放行、1 个中文约数「几十万人」标 needs_human → 审稿修订轮 300s 超时降级到最后一版过门稿（已知 P2）→ `draft_ready`。全程 1219s。成稿：亲历复盘骨架，以做插件为转折点，术语逐个翻译，无镜头标注，结尾给最小动作；补证带回新京报研究员「十分钟做贪吃蛇」等三个非程序员案例。

**待创始人裁**：成稿开头「一个不会写代码的人」是写手对创作者身份的表述（转写原话是「不是科班出身」）——身份自嘲边界目前只约束立意卡，写手层要不要同样硬约束。

**已知**：单稿 20 分钟（DeepSeek）；审稿超时治理未做；`needs_human` 数字看板可见但无人工确认入口（P1c/P2）。

## 13. P1c 落地记录（2026-09-05）

- **审稿第三类判据「立意执行」**（v3 卡时替换 v2 的角度加严块，不叠加）：审稿以主画像身份读稿；blocker ×6（主画像动作没达成 / 误区未点出或未反驳 / 收获感未兑现 / 主张不可反驳 / 机制只剩比喻 / 闯进禁区）；advisory（元素 <2 / 无最小动作 / 「[未证实]」/ needs_human 数字 / **身份资历表述**——待创始人裁，先不打回）。审稿用户消息带立意卡块；`canFindEvidence` 已由 P1b 接通。
- **调研上游**：视角校验改「只剔坏条目」——首次提交仍给一轮修复，之后剔除坏项、≥1 条合法洞察即 succeeded 并记 `partialProblems`；零合法才 `invalid_output`。受众视角新增 `inferences`（无来源心理推断，≤6，可带画像），综合与立意 pass 都以「不可作证据」标签渲染。schema 不 bump。
- 4185 测试绿、typecheck 干净、lint 0 错、`fe:build` 过。

**遗留**：首轮打回后模型不再提交的视角仍会整段作废（一条路径）；`partial` 标记未透到选题卡进度（`deep-research.ts`/`research-job-store.ts` 小改）；写手层身份表述硬约束待裁。

**合 main 阻塞**：主仓库 `main` 工作树有另一会话未提交的封面线改动（含 `frontend/src/app.css`），本分支也改了它；:4317 从主仓库启动。等封面线收口或创始人指示按 hunk 拆分。

## 14. 审稿超时治理（2026-09-05，commit 794b9b6）

根因：审稿段 5 分钟墙钟由审一遍与全部修订轮**共用**，DeepSeek 审一遍即耗尽大半，修订必超时降级——P1c 的判据在真跑里从未执行到（§12 那篇 `review: failed` 就是它）。改为总额 12 分钟 + 每轮 5 分钟封顶（`perPassDeadlineMs` 可注入），整稿墙钟 15→30 分钟。

**复跑验收**（同 §12 路径）：angles 410s → 选亲历复盘卡（锚创作者转写）→ 补证 found(4) → 写手零查证 → 数字门 0 未核验 → **审稿 passed**（首次在生产路径完整跑完）→ `draft_ready`，全程 1002s。成稿以「厂商一直锁着的门第一次没锁」为主张，做插件接审批流为转折点，术语逐个翻译，无镜头标注、无身份贬低表述。
