...
| 片 | 提交 | 证据 |
|---|---|---|
| P2a-1 配置 v2 | dda3be3 | 创始人当天 v1 备份拷到临时目录跑真迁移：两家端点、写稿/审稿/备用三指针同指 newcli、同家警告原句出现、`configured: true`；一次保存后文件变 v2 + `.v1.bak` + 0600 + 密钥保留；指向不存在端点的提交被整次拒绝 |
| P2a-2 报病 | cd722ba | 隔离目录：不存在的域名 → 「端点 dead（code.newcli.invalid）连不上：网络不通或域名解析失败。」；错 Key → 「…拒绝了 Key（401）：Key 错误或已过期，换端点没用。」；`doctor {probe:true}` 输出无 `fetch failed`、无 JSON 原文、无密钥 |
| P2b 界面 | 本片 | 隔离目录起 4399 端口真机：①空目录进首次开机卡；②错 Key →「✗ …拒绝了 Key（401）…」+「先进去再说」，进门后横幅出现；③写稿专线指向死域名 → 横幅「写稿专线：dead（…）连不上…没有备用端点，写稿会失败」，端点行「通 · 1.7s / 坏 · …/ 未测」；⑤删被引用端点 → 「删不了：… 正在被 写稿专线 使用」；⑥备用与写稿同家 → 备用块内常驻 ⚠；⑦「接入更多」五卡三段；⑧配置文件消失后发一句 → 跳回首次开机卡；主端点设死、备用 DeepSeek 发一句 → 「主模型接不上，备用 DeepSeek 顶上了」+ 回复到达，健康记录主线失败（已翻译）/ 备用成功 |

真机复盘抓到并修掉的三处（子代理交付时都有单测但真跑才露出来）：
- 探针路径没接分类器，DeepSeek 风格 `401: {…}` 带冒号信封原样漏 JSON → `describeProbeFailure`，探针按钮 / 启动全量探针 / 健康视图同一句话。
- 真实调用的 `live.error` 存原文 → 落记录时按岗位与主机名翻译（横幅取「最近一条」，live 比 probe 新时会漏）。
- 只改指针（主端点/备用/岗位）不改端点时不发健康事件 → 横幅停在旧话；现在每次保存都广播。横幅去掉记录句里重复的角色词与「已中断」尾句。

**未真机验证**：§4.3 稿卡「备用顶上」徽章需要完整写稿一轮，只有单测（`generate-fallback.test.ts`）证据。
**遗留**（不阻塞）：「接入更多」除 Telegram 外四张卡缺后端 `lastError/lastErrorAt` 字段（搜索、雷达源、公众号、生图/封面各一对）；`autocrew_workflow doctor` 不接 `dataDir` 参数，只认环境变量（自查时因此误探了生产端点一次）；`Board.tsx`/`Editor.tsx` 本片前就已超 500 行。
