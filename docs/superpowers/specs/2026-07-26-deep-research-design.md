...
## 12. R1a 验收记录（2026-07-26）

提交链 `5942e6d`(W1+W2) → `66097bd`(W3) → `bdb0f05`(W4+W5) → `9aaca0b`(冒烟修复)，全部合入 main。**tsc 干净、1841+ vitest 全绿、前端构建过、真实端到端冒烟通过**。

**首轮真实冒烟四路全挂，抓出三个打桩测不出的问题（全部修复+回归测试）**：
1. 校验语料错位：validateQuote 对原始全文校验，而 read_page 展示的是消毒后文本（URL→[链接]）——模型逐字复制仍被拒。修复：两侧走同一条消毒管线（quoteCorpus）。
2. 视角 deadline 4 分钟在真实网络+中转下三路全超时——加倍到 8 分钟。
3. W2 代理把缓存键分隔符写成真实 NUL 字节嵌进源码（运行时等价但文件对 grep 隐身）——换转义序列。

**第二轮冒烟全绿**（选题「做了一年AI Agent落地」）：四视角 2.5~5.7 分钟各自成功、综合完成、整单 6 分 12 秒、brief v1 落盘。简报质量实证：张力点直指「个人顿悟叙事 vs 61% 结构性失败数据」的编辑级洞察；6 条证据全部通过逐字校验带真实来源；19 条 gaps 诚实点名配额耗尽与缺失的一手口径；4 张素材候选（链接级，待 R1b）。

**未验收（如实声明）**：简报注入写稿的真实首跑（等创始人下一次对带简报选题写稿）；R1b 素材下载未实现。

## 13. R1b 验收记录（2026-07-27）

提交 `cf75436`（含 SSRF 骨架抽壳 guarded-fetch，fetch-external 170 例一字未动），合并 MCP topic_id 卡后 main 同步。**tsc 干净、1955+ vitest 全绿、前端构建过、真实冒烟通过**。

第三轮真实冒烟（同选题重跑）：job=partial（evidence+benchmark 成功，audience=engine_failed 瞬态、counter=invalid_output——部分成功语义与「旧简报重跑期间持续可用、落定才推指针」在日志里逐拍可见）；**brief v2 的 3 张素材候选 3/3 下载成功落库**（jpg×2+png×1，hash 命名，magic/像素双校验通过），storedCount=3，caption 自带用途标注。语义调整已落地：选题级去重+全库文件共享（三条旧测试按裁决重写）。

**命名边界**：WebP 素材可下载但导入正文被既有 PNG/JPG 门拒（该门为公众号发布兼容而存在；转码需引图像依赖，暂不做）；下载段像素闸只读格式头不解码，后续任何缩放/转码环节须自带上限。
